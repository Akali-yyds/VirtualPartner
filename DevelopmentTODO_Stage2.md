...
- [x] 阶段 2.3 已完成。
- [x] 已确认 Toki 嘴型资源和材质槽可用于 8x8 嘴型图集切换。
- [x] 已确认本阶段不做音素级口型。
...
- [x] 建立基础嘴型配置。
- [x] 建立基础表情白名单。
- [x] 支持 expression action 通过 StagePlan 调用。
- [x] 表情相关面部骨骼仍走 ActionCoordinator。
- [x] 支持文本 fallback 嘴型驱动。
- [x] 提供基础嘴型和表情调试能力。
...
- [x] 能手动测试嘴型 index 切换。
- [x] Toki 说话时嘴巴有基础开合反馈。
- [x] StagePlan expression action 能切换白名单表情。
- [x] 表情结束后能回到 Idle 面部状态。
- [x] Debug 嘴型覆盖优先级高于说话嘴型。
- [x] 本阶段没有开放 LLM 直接控制真实嘴型 index。

**验收记录**

- [x] 用户已确认 Stage 2.9 手动验收通过。
...
- [x] 阶段 2.10 已完成。
- [x] 已确认本地 TTS 服务候选和运行方式。
- [x] 已准备可用于测试的 Toki 语音素材或 voiceId。

**开发任务**

- [x] 优先测试 GPT-SoVITS 接入。
- [x] 支持真实 TTS 服务健康检查。
- [x] 支持 speech 文本生成音频并播放。
- [x] 支持 TTS 结果进入缓存。
- [x] 支持 speech action 等待真实音频播放完成。
- [x] 支持嘴型驱动接入真实音频强度。
- [x] 保持 TTS 失败时可降级。

**手动验收标准**

- [x] Toki speech 可以合成为角色语音播放。
- [x] 音频播放完成后 speech action 才完成。
- [x] 嘴型能随真实音频强度开合。
- [x] 缓存命中时能复用已有音频。
- [x] TTS 服务不可用时对话链路不崩溃。
...
- [x] 用户已确认 Stage 2.11 手动验收通过。
- [x] 已补充 GPT-SoVITS 启动说明和启动脚本，便于下次按顺序启动本地服务。
- [x] 已补充 Momotalk 气泡、场景气泡、嘴型和真实语音播放开始时机同步。
- [x] 已补充含 speech 的 stage 中身体动作等待语音实际开始后再同步启动。
- [x] 已补充 LLM StagePlan JSON 抽取兜底，避免偶发额外内容导致 JSON parse failed。
...
- [x] 阶段 2.8 已完成。
- [x] 已确认本阶段不接真实 ASR 模型。
- [x] 已确认 MockASR 可作为本阶段验收方式。

**开发任务**

- [x] 在 Momotalk 输入区加入语音入口。
- [x] 支持 Voice Mode 基础状态展示。
- [x] 支持 ASR 服务状态展示。
- [x] 支持启动、取消和结果接收的最小流程。
...
- [x] 点击语音入口能进入 Voice Mode。
- [x] MockASR 能返回一段文本。
- [x] FillInputOnly 模式下识别文本填入输入框。
- [x] AutoSendToLlm 模式下识别文本能进入发送流程。
- [x] ASR 不可用时 UI 能显示不可用状态，文本聊天不受影响。

**验收记录**

- [x] 用户已确认 Stage 2.12 手动验收通过。
- [x] 已确认当前阶段为 MockASR 流程验证，不接真实麦克风、录音、VAD 或真实 ASR 模型。
...
- [x] 阶段 2.12 已完成。
- [x] 已确认本地 ASR 服务候选和运行方式。
- [x] 已确认当前优先测试 sherpa-onnx。

**开发任务**

- [x] 优先测试 sherpa-onnx 接入。
- [x] 支持真实 ASR 服务健康检查。
- [x] 支持录音和 VAD 自动结束一轮识别。
- [x] 支持识别结果填入输入框。
- [x] 支持按配置自动发送给 LLM。
- [x] 支持 ASR 失败降级。
- [x] 如 sherpa-onnx 不满足需求，再讨论其他候选路线。

**手动验收标准**

- [x] 点击麦克风后可以开始语音输入。
- [x] 用户说话并静音后能得到识别文本。
- [x] 识别文本能填入 Momotalk 输入框。
- [x] AutoSendToLlm 模式能触发后续 StagePlan 对话。
- [x] ASR 服务失败不会影响文本聊天。

**验收记录**

- [x] 用户已确认 Stage 2.13 手动验收通过。
- [x] 已接入 sherpa-onnx 本地 ASR 服务，支持真实麦克风输入、识别结果填入输入框和 AutoSendToLlm。
- [x] 已补充服务启动预热 recognizer 和常驻麦克风输入流，避免点击麦克风后等待模型/设备打开导致丢失前几秒语音。

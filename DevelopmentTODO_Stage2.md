...
- [ ] 第一阶段阶段 0-10 已完成并通过验收。
- [ ] 已阅读 `ReadFirst.md`。
- [ ] 已阅读 `README_Stage1.md`。
- [ ] 已阅读 `virtual_partner第二阶段开发方向细节文档.md`。

**开发任务**

- [ ] 确认第二阶段总目标和明确不做内容。
- [ ] 确认第二阶段继续继承第一阶段 Runtime 原则。
- [ ] 确认后续每个阶段开始前都先单独讨论。
- [ ] 确认 TODO 只保留阶段级任务，不提前写死实现细节。
- [ ] 确认 README 阶段归档规则。

**手动验收标准**

- [ ] 根目录存在 `DevelopmentTODO_Stage2.md`。
- [ ] 根目录存在 `README.md` 文档入口。
- [ ] 根目录存在 `README_Stage1.md` 一阶段归档。
- [ ] 文档明确 StagePlan 2.0 是第二阶段主格式。
- [ ] 文档明确 timeline 1.0 迁出前必须 Git 备份或 tag。
...
- [ ] 阶段 2.0 已完成。
- [ ] 已确认当前仍以 Toki / CH0187 为主要角色。
- [ ] 已确认本阶段只做多角色架构预留，不做多角色同时在场调度。

**开发任务**

- [ ] 建立当前阶段所需的最小角色静态配置入口。
- [ ] 建立当前阶段所需的最小角色运行时上下文入口。
- [ ] 支持当前场景中的 Toki 注册为可交互角色。
- [ ] 为后续 Momotalk 联系人、Prompt、Voice、Memory 分发提供角色 id 来源。
- [ ] 保持实现轻量，不提前开发完整多角色调度系统。

**手动验收标准**

- [ ] Play 后能确认 Toki 被注册为当前可交互角色。
- [ ] 后续系统可通过角色 id 获取当前角色上下文。
- [ ] 当前 Runtime 闭环不因最小角色预留而退化。
- [ ] 未加入多角色群聊、角色间对话或多角色调度。

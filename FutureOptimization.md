...
- 状态：第一轮 PC 鼠标控制已完成，后续增强待办。
- 当前基线：右下角相机按钮进入独占镜头控制模式；支持右键旋转、中键或 `Shift + 左键` 平移、滚轮缩放、`Exit` 退出、`Reset` 重置。缩放步长、最近半径、最远半径已在 `VirtualSceneCameraInputDriver` 暴露。
- 后续可选方向：
  - 增加触摸输入 Driver：双指缩放、单指旋转或平移、UI 起点保护。
  - 增加镜头预设：默认视角、近景、俯视、角色聚焦、房间全景。
  - 增加平滑过渡：进入/退出镜头模式、Reset、Focus 时用短动画过渡。
  - 增加保存/恢复当前用户视角：退出模式保留视角，必要时持久化到本地配置。
  - 继续根据手感调整 `wheelZoomStep`、`minZoomRadius`、`maxZoomRadius`、平移边界 padding。
- 注意：镜头 UI 继续保持独立 `SceneCameraControlCanvas`，不要塞进 `MomotalkUIManager`；相机控制 API 继续保持 UI 无关。

## 场景外轮廓描边后续观察

- 状态：第一轮外轮廓描边已完成并人工确认可接受，后续以极限视角回归为主。
- 当前基线：描边采用房间整体 mask silhouette 思路，只描场景最外轮廓，不对家具、角色、房间内部边缘做逐对象描边。
- 后续可选方向：
  - 在大幅旋转、进入房间内部、极限拉近/拉远时继续观察是否出现内部描边或穿墙描边。
  - 如果需要更宽描边，评估从简单 dilation 升级到 JFA，避免大宽度采样成本过高。
  - 如果后续房间模型替换，优先更新边界代理/整体 mask 来源，不要直接回到全场景边缘检测。
  - 建立几张固定视角截图作为描边回归参考。

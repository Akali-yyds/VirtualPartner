...
- [ ] 阶段 1 已完成。
- [ ] BaseRotation 能稳定采集。
- [ ] Idle 姿态能作为后续骨骼交接的目标参考。

**开发任务**

- [ ] 建立语义骨骼到真实骨骼的映射配置。
- [ ] 支持单个语义骨骼的 x / y / z 参数调试。
- [ ] 支持 Zero，将骨骼置为相对 BaseRotation 的语义零点。
- [ ] 支持导出当前调试姿态的 JSON 参数片段。
- [ ] 预留 Debug 占用 / Release 接口，具体统一所有权交给阶段 3 实现。

**手动验收标准**

- [ ] 能选择一个语义骨骼并单独调整。
- [ ] Zero 后骨骼回到相对 BaseRotation 的语义零点。
- [ ] 能导出当前调试姿态 JSON。
- [ ] 左右骨骼能验证镜像关系，而不是同向平移式动作。
...
- [ ] 阶段 4 已完成。
- [ ] TimelinePlayer 能提交 action 请求。
- [ ] ActionCoordinator 能处理骨骼申请和释放。

**开发任务**

- [ ] 建立可调用预设动画的配置入口。
- [ ] 支持采样预设动画姿态。
- [ ] 支持 timeline 调用已登记的 animation。
- [ ] 支持预设动画被更高优先级动作抢占时整体停止。

**手动验收标准**

- [ ] 已登记动画可以通过 timeline 播放。
- [ ] 未登记动画不会播放，并能看到清晰日志。
- [ ] animation 与 bonePose 同时涉及相同骨骼时，不出现乱抢或双写。
- [ ] 动画结束后，相关骨骼能回到 Idle 或后续动作。
...
- [ ] 支持按权重抽取 FSM 动作。
- [ ] 支持 FSM 调用预设动画。
- [ ] 支持 FSM 调用 locomotion。
- [ ] 支持用户交互打断 FSM 后重新从 Idle 开始。

**手动验收标准**

- [ ] 非交互状态下，角色会等待一段随机时间后执行动作。
- [ ] FSM 动作结束后，角色回到 Idle 并重新等待。
- [ ] 用户提交消息后，FSM 停止当前动作。
- [ ] 用户交互结束后，FSM 不恢复旧动作，而是重新开始空闲循环。

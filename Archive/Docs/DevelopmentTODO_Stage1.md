...
- [x] 阶段 8 已完成。
- [x] 本地 JSON timeline 链路已经稳定。
- [x] 已明确 LLM 只输出受支持的 timeline action。

**开发任务**

- [x] 支持用户文本输入和提交。
- [x] 支持向 LLM 注入当前角色能力说明。
- [x] 支持接收 LLM 返回的 JSON timeline。
- [x] 支持校验通过后播放 LLM timeline。
- [x] 支持 latest timeline wins 的替换规则。

**手动验收标准**

- [x] 用户提交文本后，角色能显示回复气泡。
- [x] 用户提交文本后，角色能执行 LLM 返回的动作。
- [x] LLM 返回非法 action 时，合法部分仍可继续执行。
- [x] 连续提交消息时，新 timeline 准备好后能替换旧 timeline。
...
- [x] 阶段 9 已完成。
- [x] 用户输入到 LLM timeline 再到角色演出的最小闭环已跑通。
- [x] 已记录前面阶段发现的调试痛点。

**开发任务**

- [x] 补充 Runtime owner 观察能力。
- [x] 补充 Timeline 播放状态观察能力。
- [x] 补充常见错误日志。
- [x] 整理第一版演示场景。
- [x] 修复整体联调中暴露的最小必要问题。
...
- [x] 第一版最小闭环能稳定演示。
- [x] 出现非法 timeline 或非法动作时，有足够日志定位问题。
- [x] 能观察当前 timeline 播放状态和关键骨骼 owner。
- [x] 未加入第一版明确排除的功能，如 TTS、长期记忆、复杂 UI、NavMesh。

**验收记录**

- 用户已确认阶段 10 手动验收通过。
- 已确认统一 Runtime Debug 面板可观察 LLM、Timeline、FSM、Root、Bone 关键状态，旧分散面板默认隐藏但保留回退能力。
